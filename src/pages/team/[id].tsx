...
import {Modal, ModalHeader, ModalContent, ModalFooter, ModalBody} from '../../components/Modal/Modal';
...
function removeMember(groupId:string, userId:string, token:string){
    console.log(userId);
    axios
    .post(`${BACKEND_URI}/group/remove/member/${groupId}`, 
      {
        member: userId,
      },
      {
            ...
            <div>
                            ...
                            axios
                                ...
                                // si todo bien, ir al meeting
                                    if(!values.members.some(e => e._id == response.data._id) 
                                      && !groups.members.some(e => e._id == response.data._id)){
              ...
              {userId === groups.creator.userId ?
                <div style={{marginLeft:'80%'}}> 
                  <Button type="submit" onClick={() => setDisp('block')}>Add member</Button>
                </div>
               : false}
              <Container>
              <MeetingLists
                        ...
                        handleSelect={meeting => {
                            (meeting.ongoing ? Router.push(`/meeting/ongoing/${meeting.meetingId}`)
