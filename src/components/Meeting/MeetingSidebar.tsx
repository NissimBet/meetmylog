...
import {Modal, ModalHeader, ModalContent, ModalFooter, ModalBody} from '../Modal/Modal';
...
function removeResponsibility(meetingId:string, id:string, token:string){
  axios
  .post(`${BACKEND_URI}/meeting/responsability/remove/${meetingId}`, 
    {
      rId: id,
    },
    {
...
const MeetingSidebar: React.FunctionComponent<MeetingSidebarProps> = ({
  ...
  useEffect(()=>{
    ...
    for(const member of members){
      ...
      respons.filter(resp => member._id == resp.member._id).map(
        resp => (newArr[index].responsabilities.push({_id: resp._id, responsability: resp.responsability})));
      index++;
    }
  ...
  return (
    ...
    <Container className={className}>
      ...
      <ul>
        {members.map(member => (
          <li key={member.userId} onClick={() => 
                  setTag({_id: member._id , username : '@' + member.username, name:member.name, email:member.email, userId:member.userId})}>
            {member.username}
          </li>
        ))}
      </ul>
      <div style={{display : 'flex', alignItems : 'center'}}>
        ...
        {creator.userId === userId &&
        <ButtonAdd type="button" onClick={() => setCommand('/resp @') } variant='round'>
          <FaPlus style={{position: 'relative', left: '50%', margin: '-8px 0 0 -8px',}}/>
        </ButtonAdd>}
      </div>
      {memberRespons && memberRespons.map(member => (
        <div>
          ...
          <ul>
            {member.responsabilities.map(resp => (
              <Responsibility 
                >{creator.userId === userId &&
                  <EraseButton className='eraseButton' type="button" onClick={() => {setDisp('block'); setRespD({_id: resp._id, member: member.member, responsability: resp.responsability})}} variant='round'>
    ...
    <Modal display={disp}>
    <ModalContent>
      <ModalHeader title={'Do you want to remove ' + respD.member.username + `'s responsibility ` + respD.responsability + '?'} fontC={'#000'} color={'#ffffff'} handleSelect={() => setDisp('none')}/>
        <ModalFooter color={'#ffffff'}>
          <ButtonYes type="submit" onClick={() => { 
                              try{removeResponsibility(meetingId, respD._id, userToken);
